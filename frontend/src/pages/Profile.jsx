...
// ============================================================
const CACHE_KEY = 'ai_insights_cache_v3';
const DEFAULT_INSIGHT_DISCLAIMER = '内容仅供健康管理参考，不替代医生诊断。';
...
const normalizeInsight = (insight = {}) => ({
  ...insight,
  title: isQuestionMarkText(insight.title) ? '健康洞察' : insight.title,
  content: isQuestionMarkText(insight.content) ? '当前洞察内容不可用，请重新生成。' : insight.content,
  tags: Array.isArray(insight.tags) ? insight.tags.filter(tag => !isQuestionMarkText(tag)) : [],
});
...
const Profile = () => {
  ...
  const [insightsDisclaimer, setInsightsDisclaimer] = useState(DEFAULT_INSIGHT_DISCLAIMER);
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        if (hasFreshCache) {
          setDynamicAIInsights(cache.insights.map(normalizeInsight));
          setInsightsTimestamp(cache.ts);
          setInsightsDisclaimer(cache.disclaimer || DEFAULT_INSIGHT_DISCLAIMER);
          setAiGenerating(false);
          // 缓存过期只提示用户手动刷新，不在后台静默替换可见内容。
          const sixHours = 6 * 60 * 60 * 1000;
          setInsightsStale(Date.now() - cache.ts > sixHours);
        } else {
          // 没缓存或档案已变 → 拉新
  ...
  const fetchRealAIInsights = async (token, profileHash, silent = false) => {
    ...
    try {
      const res = await fetch(apiUrl('/api/profile/ai-insights'), {
        headers: { 'Authorization': `Bearer ${token}` }
      });
      const data = await res.json();
      if (data.status === 'success' && data.insights) {
        const safeInsights = Array.isArray(data.insights) ? data.insights.map(normalizeInsight) : [];
        const disclaimer = data.disclaimer || DEFAULT_INSIGHT_DISCLAIMER;
        setDynamicAIInsights(safeInsights);
        setInsightsDisclaimer(disclaimer);
        const ts = Date.now();
        ...
        writeCache(userKey, {
          ...
          disclaimer,
          generationMode: data.generation_mode || 'unknown',
              ...
              <div className="glass-card" style={{
                ...
                {aiGenerating ? (
                  <div style={{ textAlign: 'center', padding: '40px 0' }}>
                    ...
                    <Title level={4} style={{ color: PALETTE.textInk, margin: '0 0 8px 0' }}>综合健康大脑正在生成健康评估...</Title>
                    <Text style={{ color: PALETTE.textMute }}>正在基于结构化健康档案生成风险提示</Text>
                  </div>
                ) : (
                  <div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 20, flexWrap: 'wrap', gap: 12 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                        ...
                        {insightsStale && (
                          <span className="stale-pill" style={{ color: PALETTE.amber, background: PALETTE.amberSoft }}>
                            建议刷新
                          </span>
                        )}
                        <Button
                    ...
                    <div style={{ fontSize: 15, lineHeight: 1.8, color: PALETTE.textSlate }}>
                      <p style={{ marginTop: 0 }}>
                        👩‍⚕️ 尊敬的 <b style={{ color: PALETTE.tealDeep }}>{safeName}</b>，您好！基于您的结构化健康档案，后端大模型在规则护栏约束下生成了以下健康评估与风险提示：
                      </p>
                      <Text style={{ display: 'block', color: PALETTE.textMute, fontSize: 13, marginTop: -4 }}>
                        {insightsDisclaimer}
                      </Text>
                            ...
                            <Col xs={24} lg={isLast ? 24 : 12} key={index}>
                              <div className="ai-card" style={{
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16, gap: 8, flexWrap: 'wrap' }}>
                                  <Text strong style={{ color: PALETTE.textInk, fontSize: 16 }}>{safeInsight.title}</Text>
                                  <Space wrap size={[6, 6]}>
                                    {safeInsight.tags && safeInsight.tags.map((tag, ti) => (
                                      <Tag key={ti} style={{
                                ...
                                <div style={{ margin: 0, color: PALETTE.textSlate, fontSize: 14, lineHeight: 1.75 }}
                                  dangerouslySetInnerHTML={{ __html: safeInsight.content }} />
                              </div>
                            </Col>
                          );
                        })}
